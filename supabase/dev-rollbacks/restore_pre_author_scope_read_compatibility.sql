-- Development-only emergency compatibility rollback for migration 202608240004.
-- Apply only after restoring the previous dev frontend when the coordinated
-- smoke gate fails. This intentionally restores the former broad active-coach
-- read model; it does not remove additive columns, snapshots, or draft RPCs.
  ...
  select exists (
    ...
    where program.id = target_program_id
      and (
        ...
        or public.is_active_coach(program.athlete_id)
      )
  );
$$;
...
create policy profiles_read_connected
...
using (
  ...
  or exists (
    ...
    where relationship.ended_at is null
      and (
        (
          relationship.athlete_id = profiles.id
          and relationship.coach_id = (select auth.uid())
        )
        or (
          relationship.coach_id = profiles.id
          and relationship.athlete_id = (select auth.uid())
        )
      )
  )
);
...
using (
  ...
  and (
    ...
    or public.is_active_coach(athlete_id)
  )
);
...
drop policy if exists workout_sessions_read_owner on public.workout_sessions;
drop policy if exists workout_sessions_read_authorized
  on public.workout_sessions;
create policy workout_sessions_read_authorized
...
drop policy if exists session_item_logs_read_owner on public.session_item_logs;
drop policy if exists session_item_logs_read_authorized
  on public.session_item_logs;
create policy session_item_logs_read_authorized
...
using (
  exists (
    ...
    where session.id = session_item_logs.workout_session_id
      and (
        session.athlete_id = (select auth.uid())
        or public.is_active_coach(session.athlete_id)
      )
  )
);

drop policy if exists session_entries_read_owner on public.session_entries;
drop policy if exists session_entries_read_authorized on public.session_entries;
create policy session_entries_read_authorized
...
-- Restore the legacy session-save surface only so the previous frontend can
-- operate while the failed release is investigated. Item-log snapshots remain
-- RPC-owned because the old frontend does not mutate them.
