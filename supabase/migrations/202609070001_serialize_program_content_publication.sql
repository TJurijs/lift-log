-- Every content writer, including direct PostgREST writes, participates in the
-- same version lock as publication. A writer that loses the race must recheck
-- the version after waiting and must never modify the published snapshot.
...
begin
  -- Preserve the existing service-only namespace fixture cleanup contract.
  if tg_op = 'DELETE'
  ...
  if tg_op <> 'INSERT' then
    old_version := private.resolve_tree_version(
      tg_table_name, (to_jsonb(old) ->> parent_column)::uuid
    );
    if old_version is null then raise exception 'Program version was not found'; end if;
  end if;
  if tg_op <> 'DELETE' then
    new_version := private.resolve_tree_version(
      tg_table_name, (to_jsonb(new) ->> parent_column)::uuid
    );
    if new_version is null then raise exception 'Program version was not found'; end if;
  end if;

  expected_count := case
    when old_version is null or new_version is null or old_version = new_version then 1
    else 2
  end;
  -- SHARE permits unrelated edits in one draft, but conflicts with the UPDATE
  -- lock used by publication and copying. Sort cross-version moves consistently.
  for locked_version in
    select version.id, version.status
    from public.program_versions version
    where version.id in (old_version, new_version)
    order by version.id
    for share
  loop
  ...
  if locked_count <> expected_count then raise exception 'Program version was not found'; end if;
  if tg_op = 'DELETE' then return old; end if;
  return new;
end;
$$;

revoke all on function private.assert_draft_program_tree() from public, anon, authenticated;

-- Append serialization belongs to the parent week/section. SHARE on the
-- version cooperates with the common tree guard without lock upgrades.
