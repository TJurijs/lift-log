-- Use a simple bodyweight core movement to finish Elina's workout.
...
begin
  ...
  where exercise.scope = 'global'
    ...
    and lower(trim(exercise.name)) = 'dead bug'
  order by
  ...
  where program.athlete_id = program.created_by_id
    ...
    and lower(trim(item.snapshot_name)) = 'band external rotation'
  limit 1;

  if target_item_id is null then return; end if;
  if replacement.id is null then raise exception 'Video-backed Dead Bug exercise is missing'; end if;
  ...
  select target_item_id, position - 1, 10, 10, 4, 4
