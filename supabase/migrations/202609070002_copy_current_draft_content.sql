-- A duplicate of a mutable draft is a new independent draft. Immutable lineage
-- must continue to point only to an immutable revision, never to a working copy.
...
begin
  ...
  where program.id = target_program_id and program.archived_at is null
  for share;

  -- This exclusive version lock waits for earlier tree edits and prevents new
  -- edits/publication until the complete copy has been read in this transaction.
  ...
  order by case version.status when 'draft' then 0 when 'published' then 1 else 2 end,
  ...
  insert into public.programs (
    ...
    source_program.planning_mode, true, 'self', 'Duplicated by you', source_program.content_type
  ...
  insert into public.program_versions (
    ...
    new_program_id, current_user_id,
    case when source_version.status = 'draft' then null else source_version.id end,
    1, 'draft'
