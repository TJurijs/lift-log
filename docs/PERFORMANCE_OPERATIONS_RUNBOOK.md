...
| --- | --- | ---: |
| `program-10w-40` | 10 weeks, 40 workouts, nested sections/items/prescriptions | 1,453 |
| `program-52w-stress` | 52 weeks, 208 workouts | 7,543 |
...
The `local-supabase` CI job resets and lints the disposable database, runs the integration suite and V1 rollback smoke, captures the 10-week/40-workout plus 50-athlete/150-session scale plans, seeds the local browser personas, then gates the read-only runtime report. Its local password is fixture-only and has no meaning outside the ephemeral CI stack.
...
2. On disposable local Supabase, exercise 1 athlete and the 10-week program. Establish bootstrap/navigation/detail request counts and query timings.
...
5. Stress one 52-week program: open, navigate, schedule, and clone sequentially. Assert request counts remain O(1) per screen; separately time the local write transaction and cleanup.
