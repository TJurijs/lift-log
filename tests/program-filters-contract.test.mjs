...
test("Programs expose reusable-content filters without run-status filtering", async () => {
  ...
  assert.match(app, /Type[\s\S]*Programs[\s\S]*Workouts/);
...
test("mobile program cards align status and actions in stable rows", async () => {
  ...
  assert.match(app, /className="program-card-more"/);
  assert.match(app, /className="button primary small program-card-action-schedule"/);
});
...
test("mobile program editing reserves aligned controls and one exercise list", async () => {
  ...
  assert.match(styles, /\.builder-exercise-preview \.item-position(?:\s*,[^{}]*)?\s*\{[^}]*display: none/s);
});
