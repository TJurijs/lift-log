...
test("scale fixtures are deterministic and cover approved load shapes", () => {
  ...
  assert.equal(first.totalRows, 268541);
  assert.deepEqual(first.scenarios.map(({ name, totalRows }) => [name, totalRows]), [
    ["program-40", 1364], ["program-208-stress", 7076], ["coach-50x150", 255101], ["exercise-5000", 5000],
  ]);
  assert.equal(
    first.scenarios.find(({ name }) => name === "program-40").sectionsPerWorkout,
    1,
  );
