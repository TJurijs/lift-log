...
test("destructive confirmations always use the shared application overlay", async () => {
  ...
  assert.ok(
    [...app.matchAll(/setModal\("delete-content"\)/g)].length >= 4,
    "program, program-run, workout, and workout-item deletion must open the shared overlay",
  );
  assert.match(app, /function DeleteContentModal[\s\S]*<ModalShell/);
  assert.match(app, /kind: "workout"[\s\S]*kind: "workout-item"[\s\S]*kind: "program-run"[\s\S]*kind: "program"/);
