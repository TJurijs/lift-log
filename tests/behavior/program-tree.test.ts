...
describe("program tree ordering", () => {
  ...
  it("reorders only the requested workout's sections", () => {
    const source = program();

    const result = reorderProgramSections(source, "workout-1", [
      "section-b",
      "section-a",
    ]);

    expect(result.weeks[0].workouts[0].sections.map(({ id }) => id)).toEqual([
      "section-b",
      "section-a",
    ]);
  ...
  it("moves an exercise between sections and clamps the destination position", () => {
    const source = program();
    source.weeks[0].workouts[0].sections[0].items = [item("item-a")];
    source.weeks[0].workouts[0].sections[1].items = [item("item-b")];
    ...
    expect(result.weeks[0].workouts[0].sections[0].items).toEqual([]);
    expect(
      result.weeks[0].workouts[0].sections[1].items.map(({ id }) => id),
    ).toEqual(["item-b", "item-a"]);
    expect(source.weeks[0].workouts[0].sections[0].items).toHaveLength(1);
  });

  it("returns the same program when the exercise is absent", () => {
    const source = program();

    expect(
      moveProgramExercise(source, "workout-1", "missing", "section-b", 0),
    ).toBe(source);
  });
});
