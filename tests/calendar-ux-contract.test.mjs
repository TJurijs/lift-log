...
test("calendar event clicks open plans and immutable completed results", async () => {
  ...
  assert.match(
    app,
    /function openCalendarPlan[\s\S]*openWorkoutPreview\(schedule, "calendar", false\)[\s\S]*setActiveView\("workout"\)[\s\S]*pushAppDetailHistory\("workout", "workout"\)/,
    "planned calendar events must use the shared full workout preview",
  );
  assert.match(
    app,
    /function openCalendarResults[\s\S]*pushAppDetailHistory\("workout-log", "workout"[\s\S]*kind: "workout-log"[\s\S]*session[\s\S]*restoreCompletedWorkoutFromHistory/,
...
test("Training has no separate template route and workouts can be repeated", async () => {
  const [app, styles] = await Promise.all([
    readAuthoringSource(),
    readFile(stylesUrl, "utf8"),
  ]);
